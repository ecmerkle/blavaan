lav2stancond <- function(model, lavdata = NULL, dp = NULL, n.chains = 1, mcmcextra = "", inits = "prior", debug = FALSE) {
  ...
  if(debug){
    ...
    capture.output(out$model <- rstan::stanc_builder(file = tmp, isystem = isystem,
                                                     obfuscate_model_name = TRUE)$model_code)
  }
